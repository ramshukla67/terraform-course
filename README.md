...
Instead of writing all infrastructure code from scratch or using local modules, this example shows how to reference external modules maintained by third parties or your organization.
...
- **providers.tf** - Defines the AWS provider configuration (region: eu-west-1) and required provider versions (~> 5.0)
- **networking.tf** - Instantiates the external `udemy-lauromueller/networking-tf-course/aws` module (version 0.1.1) with VPC and subnet configurations
- **.terraform.lock.hcl** - Lock file automatically generated by `terraform init` to pin provider versions for reproducibility
...
```bash
terraform init    # Downloads the module and initializes the working directory
terraform plan    # Shows what resources will be created
terraform apply   # Creates the VPC and subnets on AWS
```
